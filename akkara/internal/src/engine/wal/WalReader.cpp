...
public:
    ...
    [[nodiscard]] static FileReader open(const std::filesystem::path& path) {
        ...
        if (reader.handle_ == INVALID) {
            throw std::runtime_error("Failed to open WAL file: " + path.string());
        }

        LARGE_INTEGER size;
        if (::GetFileSizeEx(reader.handle_, &size)) {
            reader.file_size_ = static_cast<uint64_t>(size.QuadPart);
        }
#else
        ...
        struct stat st;
        if (::fstat(reader.handle_, &st) == 0) {
            reader.file_size_ = static_cast<uint64_t>(st.st_size);
        }
#endif

        return reader;  // ← move constructorが呼ばれる（C++17以降はRVO/NRVOで最適化される）
    }
...
public:
    ...
    [[nodiscard]] std::optional<WalOp> next() {
        ...
        if (read_buffer_.size() < frame_len) {
            read_buffer_ = core::OwnedBuffer::allocate(frame_len, 4096);
        }

        std::memcpy(read_buffer_.data(), header_buf, WalFraming::FRAME_HEADER_SIZE);

        const size_t body_read = file_reader_.read(
            reinterpret_cast<uint8_t*>(read_buffer_.data()) + WalFraming::FRAME_HEADER_SIZE,
            remaining_len
        );
    ...
    [[nodiscard]] bool has_error() const noexcept {
        return error_type_ != ErrorType::NONE;
    }
...
std::unique_ptr<WalReader> WalReader::open(const std::filesystem::path& wal_file) {
    return std::unique_ptr<WalReader>(new WalReader(wal_file));
}

WalReader::WalReader(const std::filesystem::path& wal_file)
    : impl_{std::make_unique<Impl>(wal_file)} {}

WalReader::~WalReader() = default;

std::optional<WalOp> WalReader::next() {
    return impl_->next();
}

bool WalReader::has_error() const noexcept {
    return impl_->has_error();
}

WalReader::ErrorType WalReader::error_type() const noexcept {
    return impl_->error_type();
}

uint64_t WalReader::error_position() const noexcept {
    return impl_->error_position();
}

uint64_t WalReader::current_position() const noexcept {
    return impl_->current_position();
}

uint64_t WalReader::file_size() const noexcept {
    return impl_->file_size();
}

uint64_t WalReader::operations_read() const noexcept {
    return impl_->operations_read();
}
