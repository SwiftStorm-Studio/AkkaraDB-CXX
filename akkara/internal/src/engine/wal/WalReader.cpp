...
namespace akkaradb::engine::wal {
    ...
    public:
        ...
        inline static const auto INVALID = INVALID_HANDLE_VALUE;
#else
        ...
        FileReader(FileReader&& other) noexcept : handle_{other.handle_}, file_size_{other.file_size_} {
        ...
        [[nodiscard]] static FileReader open(const std::filesystem::path& path) {
            ...
            if (reader.handle_ == INVALID) { throw std::runtime_error("Failed to open WAL file: " + path.string()); }

            LARGE_INTEGER size;
            if (::GetFileSizeEx(reader.handle_, &size)) { reader.file_size_ = static_cast<uint64_t>(size.QuadPart); }
#else
...
#ifdef _WIN32
            DWORD bytes_read = 0;
            if (!::ReadFile(handle_, buffer, static_cast<DWORD>(size), &bytes_read, nullptr)) { return 0; }
            return bytes_read;
#else
    ...
    public:
        explicit Impl(const std::filesystem::path& wal_file) : file_reader_{FileReader::open(wal_file)}
