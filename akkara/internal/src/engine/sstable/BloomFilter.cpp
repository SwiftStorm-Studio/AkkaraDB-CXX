...
namespace akkaradb::engine::sstable {
    namespace {
        ...
        [[nodiscard]] constexpr uint64_t rotl64(uint64_t x, int b) noexcept {
            return (x << b) | (x >> (64 - b));
        }
...
#undef SIPHASH_ROUND
        ...
        [[nodiscard]] bool is_pow2(uint32_t x) noexcept {
            return x > 0 && (x & (x - 1)) == 0;
        }
        ...
        [[nodiscard]] uint8_t optimal_hashes(uint32_t m_bits, uint64_t n) noexcept {
            ...
            return static_cast<uint8_t>(
                (std::max)(2.0, (std::min)(16.0, std::round(k)))
            );
        }
    ...
    BloomFilter::BloomFilter(
        uint32_t m_bits, uint8_t k, uint64_t seed, std::vector<uint64_t> words
    ...
    BloomFilter BloomFilter::read_from(core::BufferView buffer) {
        if (buffer.size() < HEADER_SIZE) {
            throw std::runtime_error("BloomFilter: buffer too small for header");
        }
        ...
        if (!is_pow2(m_bits) || m_bits < 64) {
            throw std::runtime_error("BloomFilter: mBits must be power-of-2 >= 64");
        }
        ...
        if (buffer.size() < HEADER_SIZE + body_bytes) {
            throw std::runtime_error("BloomFilter: buffer too small for bits");
        }

        std::vector<uint64_t> words(body_bytes / 8);
        const uint8_t* bits_data = data + HEADER_SIZE;
        for (size_t i = 0; i < words.size(); ++i) {
            std::memcpy(&words[i], bits_data + i * 8, 8);
        }
    ...
    core::OwnedBuffer BloomFilter::serialize() const {
        ...
        for (size_t i = 0; i < words_.size(); ++i) {
            std::memcpy(bits_data + i * 8, &words_[i], 8);
        }
        return buffer;
    }
    ...
    uint64_t BloomFilter::fingerprint64(
        std::span<const uint8_t> key, uint64_t seed
    ) noexcept {
    ...
    BloomFilter::Builder BloomFilter::Builder::create(
        uint64_t expected_insertions, double fp_rate, uint64_t seed
    ) {
        if (expected_insertions == 0) { expected_insertions = 1; }
        if (fp_rate <= 0.0 || fp_rate >= 0.5) {
            throw std::invalid_argument("BloomFilter: fp_rate out of range");
        }
    ...
    BloomFilter::Builder::Builder(uint32_t m_bits, uint8_t k, uint64_t seed)
        : m_bits_{m_bits}, mask_{m_bits - 1}, k_{k}, seed_{seed}
        , words_(m_bits / 64, 0)
    {
    ...
    BloomFilter BloomFilter::Builder::build() {
        return BloomFilter{m_bits_, k_, seed_, std::move(words_)};
    }
