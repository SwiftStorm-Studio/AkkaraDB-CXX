...
namespace akkaradb::format::akk {
        ...
        public:
            ...
            void begin_block() {
                if (current_buffer_.empty()) {
                    ...
                    assert(current_buffer_.data() != nullptr && "PooledBuffer::acquire returned null data");
                    ...
                    assert(current_buffer_.view().data() != nullptr);
            ...
            bool try_append(std::span<const uint8_t> key, std::span<const uint8_t> value, uint64_t seq, uint8_t flags, uint64_t key_fp64, uint64_t mini_key) {
                assert(!current_buffer_.empty());
                if (current_buffer_.empty()) {
    ...
    std::unique_ptr<AkkBlockPacker> AkkBlockPacker::create(BlockReadyCallback callback, std::shared_ptr<core::BufferPool> pool) {
        return std::unique_ptr<AkkBlockPacker>(new AkkBlockPacker(std::move(callback), std::move(pool)));
    }

    AkkBlockPacker::AkkBlockPacker(BlockReadyCallback callback, std::shared_ptr<core::BufferPool> pool)
    ...
    bool AkkBlockPacker::try_append(const core::MemRecord& record) {
        ...
        return try_append(record.key(), record.value(), hdr.seq, hdr.flags, hdr.key_fp64, hdr.mini_key);
    }
