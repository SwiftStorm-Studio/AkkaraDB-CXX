 ...
 */
    ...
    private:
        void flush_internal() {
            ...
            // Build data_views for parity encoding
            std::vector<core::BufferView> data_views;
            data_views.reserve(pending_blocks_.size());
            for (const auto& block : pending_blocks_) { data_views.push_back(block.view()); }

            // Encode parity blocks
            std::vector<core::OwnedBuffer> parity_blocks;
            if (m_ > 0) { parity_blocks = parity_coder_->encode(data_views); }

            // Build Stripe
            Stripe stripe{
                .data_blocks = std::move(pending_blocks_),
                .parity_blocks = std::move(parity_blocks),
                .stripe_id = stripe_id_++
            };
    ...
    std::unique_ptr<AkkStripeWriter> AkkStripeWriter::create(
        ...
        return std::unique_ptr<AkkStripeWriter>(new AkkStripeWriter(
            k, m, std::move(parity_coder), std::move(callback), flush_policy
        ));
    }

    AkkStripeWriter::AkkStripeWriter(
        size_t k,
        size_t m,
        std::shared_ptr<ParityCoder> parity_coder,
        StripeReadyCallback callback,
