 ...
 */
     ...
     */
        struct Footer {
            ...
            Footer(uint64_t idx_off, uint64_t blm_off, uint32_t cnt, uint8_t ver = VERSION) : index_off{idx_off}
                                                                                              , bloom_off{blm_off}
                                                                                              , entries{cnt}
                                                                                              , version{ver} {}
        };
     ...
     */
        [[nodiscard]] static core::OwnedBuffer write_to_buffer(
            const Footer& footer,
            uint32_t crc32c = 0
        );
     ...
     */
        [[nodiscard]] static Footer read_from_file(
            core::BufferView buffer,
    ...
    private:
        static uint32_t compute_file_crc(
            const std::filesystem::path& file_path,
            uint64_t file_size
        );
    };
