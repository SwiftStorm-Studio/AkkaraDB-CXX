 ...
 * [magic:u32][payloadLen:u32][payload][crc32c:u32]
 *
 * - magic: 0x57414C46 ("WALF" in ASCII)
 * - payloadLen: Length of payload in bytes
 * - payload: Serialized WalOp
 * - crc32c: CRC32C checksum over [magic][payloadLen][payload]
 *
 * Design principles:
 * - Self-describing: Magic + length for parsing
