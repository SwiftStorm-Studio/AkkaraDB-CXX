...
namespace akkaradb::core {
    ...
    struct OwnedRecord {
        ...
        [[nodiscard]] int compare_key(const OwnedRecord& other) const noexcept {
            const size_t prefix = std::min<size_t>(std::min(hdr.k_len, other.hdr.k_len), 8u);

            if (prefix > 0) { if (int c = std::memcmp(&mini_key, &other.mini_key, prefix); c != 0) return c < 0 ? -1 : 1; }
        ...
        [[nodiscard]] int compare_key(std::span<const uint8_t> other) const noexcept {
            ...
            if (prefix > 0) { if (int c = std::memcmp(&mini_key, other.data(), prefix); c != 0) return c < 0 ? -1 : 1; }
