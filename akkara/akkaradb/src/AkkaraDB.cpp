...
namespace akkaradb {
    ...
    std::span<const uint8_t> Record::key() const noexcept { return impl_->key(); }

    std::span<const uint8_t> Record::value() const noexcept { return impl_->value(); }

    uint64_t Record::seq() const noexcept { return impl_->seq(); }

    uint8_t Record::flags() const noexcept { return impl_->flags(); }

    bool Record::is_tombstone() const noexcept { return impl_->is_tombstone(); }

    size_t Record::approx_size_bytes() const noexcept { return impl_->approx_size_bytes(); }
    ...
    std::unique_ptr<AkkaraDB> AkkaraDB::open(const Options& opts) {
        ...
        if (!internal_db) { throw std::runtime_error("AkkaraDB: failed to open database"); }
    ...
    AkkaraDB::~AkkaraDB() noexcept { if (impl_) { impl_->close(); } }
    ...
    void AkkaraDB::close() noexcept { if (impl_) { impl_->close(); } }
} // namespace akkaradb