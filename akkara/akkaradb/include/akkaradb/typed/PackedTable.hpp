...
namespace akkaradb::typed {
    ...
    [[nodiscard]] constexpr uint64_t rotl64(uint64_t x, int r) noexcept {
        return (x << r) | (x >> (64 - r));
    }
    ...
    [[nodiscard]] inline uint64_t murmur3_hash(std::string_view str) noexcept {
        ...
        if (k1 != 0) { k1 *= c1; k1 = rotl64(k1, 31); k1 *= c2; h1 ^= k1; }
        if (k2 != 0) { k2 *= c2; k2 = rotl64(k2, 33); k2 *= c1; h2 ^= k2; }
    ...
    private:
        ...
        std::vector<uint8_t> serialize_entity(const Entity& entity) {
            ...
            std::apply([&](auto... ptrs) {
                estimated_size = (estimate_size(entity.*ptrs) + ...);
            }, field_ptrs);
            buf.reserve(estimated_size);

            std::apply([&](auto... ptrs) {
                (serialize(buf, entity.*ptrs), ...);
            }, field_ptrs);
    ...
    template <typename Entity, typename Key>
    PackedTable<Entity, Key>::PackedTable(AkkaraDB& db, std::string_view table_name)
    ...
    template <typename Entity, typename Key>
    uint64_t PackedTable<Entity, Key>::put(const Key& key, const Entity& entity) {
        return impl_->put(key, entity);
    }

    template <typename Entity, typename Key>
    std::optional<Entity> PackedTable<Entity, Key>::get(const Key& key) {
        return impl_->get(key);
    }

    template <typename Entity, typename Key>
    uint64_t PackedTable<Entity, Key>::remove(const Key& key) {
        return impl_->remove(key);
    }

    template <typename Entity, typename Key>
    template<typename ExprType>
    std::vector<Entity> PackedTable<Entity, Key>::query(const query::Expr<ExprType>& expr) {
        return impl_->query(expr);
    }
